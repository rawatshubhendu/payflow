...
export default function AnalyticsPage() {
  ...
  const ready =
    overview !== null &&
    subscription !== null &&
    (!canCharts || (revenue !== null && paymentStatus !== null));
  ...
  return (
    <AppShell title="Analytics" description="Understand collections, revenue and overdue money">
      {error ? (
        <div className="mb-4 rounded-xl border border-red-200 bg-red-50/70 p-3 text-sm text-red-700">
          {error}
        </div>
      ) : null}
          ...
          {canCharts && revenue && paymentStatus ? (
            ...
            <div className="mt-4 grid gap-4 lg:grid-cols-2">
              <ProFeatureCard
                ...
                description="See revenue trends over 7, 30 or 90 days with the collected-from-verified-payments series."
              />
              <ProFeatureCard
                title="Payment status"
                description="Visualize how much is collected, pending, overdue and failed across your invoices."
              />
            </div>
          )}
        </>
